...
interface Props {
	comment: Comment;
	history: any;
	onChangeComment: (evt: ChangeEvent<HTMLTextAreaElement>) => void;
	onChangeRating: (evt: ChangeEvent<HTMLInputElement>) => void;
	onSubmitReview: (evt: FormEvent<HTMLFormElement>) => void;
	selectedFilm: Film;
	sendingComment: {
...
const AddReview: FC<Props> = (props: Props) => {
	const {
		comment,
		history,
		onChangeComment,
	...
	const isSendingReview = () => {
		...
		if (
			sendingComment.commentsIsSending &&
			!sendingComment.sendingIsError
		) {
			return ``;
		}
	...
	const isBlocked =
		sendingComment.commentsIsSending && !sendingComment.sendingIsError;

	return (
		<section
			className="movie-card movie-card--full"
			style={{ backgroundColor: selectedFilm.bgc }}
		>
					...
					<div className="add-review__text">
						...
						<div className="add-review__submit">
							<button
								...
								type="submit"
								disabled={isValidReview || isBlocked}
							>
								Post
							</button>
						</div>
