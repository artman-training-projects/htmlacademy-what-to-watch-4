...
const Operations = {
	...
	loadFavoriteFilms: () => (dispatch, getState, api) => {
		return api
			.get(EntryPoints.FAVORITE)
			.then((response) => {
				dispatch(
					ActionCreator.loadFavoriteFilms(
						response.data.map((film) => filmAdapter(film)),
					),
				);
	...
	sendFavoriteFilm: (filmID, isFavorite) => (dispatch, getState, api) => {
		...
		return api
			.post(`${EntryPoints.FAVORITE}/${filmID}/${status}`, {
